      ...
      <CarouselContent>
        {infoProjects.map((x, i) => (
          <CarouselItem key={i} className="md:basis-1/2 lg:basis-1/2">
            <div className="dark:bg-gray-900/50 backdrop-blur-sm rounded-2xl p-4 bg-white  h-[400px]">
          ...
          ariaLabel="Zoom Area"
        >
                <img src={x.photoProj} alt={x.photoProj} className="h-[210px] w-full" />
                </Lens>
              </figure>
              <div className="mt-4 flex justify-between items-center ">
                <a href={x.link}>
              <Button variant="link" className="text-gray-400">{x.linkName}</Button></a>
<AvatarCircles
  avatarUrls={
    x.People?.map(p => ({
      imageUrl: p.imageUrl,
